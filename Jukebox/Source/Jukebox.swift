...
// Jukebox.swift
//
// Copyright © 2016 Cristian Tabuyo
//
// The base of this software was gotten from a free & opensource Github repo
// however it´s been hugely edited and improved to meet @ViroMusicApp quality
// standards by the back-end main programmer @Cristian Tabuyo. This project may
// not be used under any circumstance since it no longer is free and/or opensource
// ilegally using this edited & improved software is and will considered as a legal
// wrongdoing which is subject to legal issues. Having being warned about the
...
extension Jukebox {
    ...
    public func setIT(URL: [JukeboxItem]) {
        setItems(items: URL)
        NotificationCenter.default.addObserver(self, selector: #selector(Jukebox.updateShit), name: NSNotification.Name(rawValue: "updateInfoCenterJukebox"), object: nil)
    }
...
open class Jukebox: NSObject, JukeboxItemDelegate {
    ...
    open var currentItem: JukeboxItem? {
        if  let q = queuedItems {
            guard playIndex >= 0 && playIndex < q.count else {
                return nil
            }
           return q[playIndex]
        }
        return JukeboxItem.init(URL: URL(string: "http://google.es")!)
    }
    ...
    fileprivate func updateInfoCenter() {
        ...
        if let h = Image {
            nowPlayingInfo[MPMediaItemPropertyArtwork] = MPMediaItemArtwork(image: h)
        }
    ...
    func playerItemDidPlayToEnd(_ notification : Notification){
         delegate?.audioPlayerDidFinishPlaying(player: self, successfully: true)
